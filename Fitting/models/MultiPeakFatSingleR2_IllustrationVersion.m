...
function signalStruct = MultiPeakFatSingleR2_IllustrationVersion(t,tesla,F,W,R2star,fB);
...
%Note that demo version reverses frequencies for illustration purposes
...
Fatshift = -Fatshift; % Reverses fat shift for demo purposes
Fatw= Fatshift*larmor*2*pi/1000; %Fatw is angular frequency

Watershift = 0;
Waterw=Watershift*larmor*2*pi/1000; %Waterw is angular frequency

%For illustration version, create separate variables to allow separate
%display

%Water
Sw = W*exp((i*Waterw)*t');

for k = 1:numel(Fatamps)

%Multipeak fat 
Sfmulti(k,:) = F*Fatamps(k)*exp((i*Fatw(k))*t')

end

%Single peak fat 
Sfsingle = F*exp((i*Fatw(2))*t')
...
Sw=Sw.*exp(-R2star*t'); %R2star term
Sw=Sw.*exp(i*2*pi*fB*t'); %B0 inhomogeneity

Sfsingle=Sfsingle.*exp(-R2star*t'); %R2star term
Sfsingle=Sfsingle.*exp(i*2*pi*fB*t'); %B0 inhomogeneity

Sfmulti=Sfmulti.*exp(-R2star*t'); %R2star term
Sfmulti=Sfmulti.*exp(i*2*pi*fB*t'); %B0 inhomogeneity
...
signalStruct.Ssingle = Sw + Sfsingle;
signalStruct.Smulti = S;
signalStruct.Sw = Sw;
signalStruct.Sfsingle = Sfsingle;
signalStruct.Sfmulti = Sfmulti;
signalStruct.SfmultiNet = sum(Sfmulti,1);
